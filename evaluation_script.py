...
def storePredictions(y_test, pred, y_score, filename):
    pd.DataFrame(y_test).to_csv(filename + "_trueValues.csv")
    pd.DataFrame(pred).to_csv(filename + "_predValues.csv")
    pd.DataFrame(y_score).to_csv(filename + "_probValues.csv")



def Evaluation(d, dataSetName, filename, y_test, pred, y_score, x, removedValues, timeForEval, methodName, trainTime, testTime):
    outputDictionary = {}
    ...
    outputDictionary["trainTime"] = timeForEval - testTime
    outputDictionary["testTime"]  = testTime
    ...
    storePredictions(y_test, pred, y_score, filename)
