...
def is_sparse(dataset_name):
    with open(dataset_name + "_train.arff") as f:
        for x in f:
        ...
        for x in f:
            ...
            if y:
                if not y.startswith("{"):
                    print("File: ", arff)
                    print("This is not sparse arff, current line:")
                    print(y)
                    return False
    return True
...
def read_dataset(dataset_name):
    ...
    if targetIndex > 0:
        ...
        featuresNames = pd.DataFrame(train["attributes"][targetIndex:])
    else:
        print("Na kraj sum")
        X_train = pd.DataFrame(train["data"])
        y_train = X_train.iloc[:nTrain, targetIndex:].copy()
    ...
    isSparse = is_sparse(dataset_name)

    return [X_train, X_test, y_train, y_test, targetIndex, featuresNames, isSparse]
...
def read_dataset_fold(dataset_name, number):
    ...
    try:
        ...
        saveout = sys.stdout
        saveerr = sys.stderr
        logFile = open(dataset_name+"_out_"+".log", "w")
        sys.stdout = logFile
        print("The dataset " + dataset_name + " was not readed properly")
        errorFile = open("error_"+dataset_name+".log", "w")
        sys.stderr = errorFile
